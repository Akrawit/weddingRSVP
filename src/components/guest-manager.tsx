...
export function GuestManager({ initialGuests, setupRequired = false, siteOrigin }: { initialGuests: AdminGuest[]; setupRequired?: boolean; siteOrigin?: string }) {
  ...
  async function setVipStatus(guest: AdminGuest, isVip: boolean) {
    if (mutation.current) return;
    mutation.current = true; setError(''); setNotice('');
    setGuests(current => current.map(row => row.id === guest.id ? { ...row, is_vip: isVip } : row));
    try {
      const response = await fetch('/api/admin/guests', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: guest.id, is_vip: isVip }) });
      const data = await response.json();
      if (!response.ok) { if (response.status === 401) router.refresh(); throw new Error(data.error); }
      setNotice(`${guest.display_name} ${isVip ? 'marked as VIP' : 'removed from VIP'}.`);
    } catch (e) {
      setGuests(current => current.map(row => row.id === guest.id ? { ...row, is_vip: guest.is_vip } : row));
      setError(e instanceof Error ? e.message : 'Could not update the VIP status. Please try again.');
    } finally { mutation.current = false; }
  }
  ...
  return <main className="admin-page">
    ...
    <section className="guest-list"><div className="guest-list-heading"><h2>Guest list</h2><div><button disabled={setupRequired || busy} onClick={() => { setBusy(true); refresh().then(() => setError('')).catch(e => setError(e.message)).finally(() => setBusy(false)); }}>Refresh</button><button disabled={setupRequired} onClick={() => fileInput.current?.click()}>Import CSV</button>{!setupRequired ? <a href="/api/admin/guests?format=csv">Export CSV ↓</a> : <button disabled>Export CSV ↓</button>}</div></div><input type="file" ref={fileInput} accept=".csv,text/csv" className="sr-only" tabIndex={-1} onChange={e => readFile(e.target.files?.[0])} aria-label="Import guest CSV" />
      ...
      <div className="guest-table-wrap"><table><thead><tr><th>Guest / party</th><th>VIP</th><th>RSVP</th><th>Attending</th><th>Dietary needs</th><th>Table</th><th>Invitation</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map(guest => <tr key={guest.id}><td><strong>{guest.display_name}</strong>{guest.plus_one_name && <small>With {guest.plus_one_name}</small>}<small>{guest.preferred_language === 'th' ? 'Thai' : 'English'}</small></td><td className="vip-cell"><label><input type="checkbox" checked={guest.is_vip} onChange={event => setVipStatus(guest, event.target.checked)} /><span className="sr-only">Mark {guest.display_name} as VIP</span></label></td><td><span className={`rsvp-badge ${guest.rsvp_status}`}>{guest.rsvp_status}</span></td><td className="attendee-cell">{guest.rsvp_status === 'accepted' ? guest.seats_confirmed : '—'}</td><td className="dietary-cell">{guest.dietary_requirement || '—'}</td><td>{guest.table_number || '—'}</td><td className="invitation-cell"><span className={`sent-badge ${guest.invitation_sent ? 'sent' : ''}`}>{guest.invitation_sent ? 'Sent' : 'Not sent'}</span><button onClick={() => copyInvitation(guest)}>Copy LINE invite ↗</button>{guest.invitation_sent && <button className="undo-sent" onClick={() => setInvitationSent(guest, false, `${guest.display_name} marked not sent.`)}>Undo sent</button>}</td><td className="row-actions"><button aria-label={`Edit ${guest.display_name}`} onClick={() => edit(guest)}>Edit</button><button aria-label={`Delete ${guest.display_name}`} onClick={() => { setError(''); setDeleting(guest); }}>Delete</button></td></tr>)}</tbody></table></div>
