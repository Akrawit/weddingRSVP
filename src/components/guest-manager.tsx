...
const empty: GuestInput = { display_name: '', seats_allocated: 1, preferred_language: 'en', plus_one_allowed: false, table_number: null };
export function GuestManager({ initialGuests, setupRequired = false, siteOrigin }: { initialGuests: AdminGuest[]; setupRequired?: boolean; siteOrigin?: string }) {
  ...
  return <main className="admin-page">
    ...
    <section className="headcount-grid" aria-label="RSVP totals"><div className="headcount-main"><p>CONFIRMED GUESTS</p><strong>{metric(totals.confirmedPeople)}</strong><span>people attending</span><p className="headcount-note">{setupRequired ? 'Waiting for connection' : `From ${totals.accepted} accepted invitation${totals.accepted === 1 ? '' : 's'}`}</p></div><div className="headcount-secondary"><div><span>Invited people</span><strong>{metric(totals.invitedPeople)}</strong><small>{setupRequired ? 'Allocated seats' : `${totals.invitations} invitations`}</small></div><div><span>Accepted</span><strong>{metric(totals.accepted)}</strong><small>invitations</small></div><div><span>Still waiting</span><strong>{metric(totals.waiting)}</strong><small>{setupRequired ? 'invitations' : `${totals.waitingPeople} possible guests`}</small></div><div><span>Declined</span><strong>{metric(totals.declined)}</strong><small>invitations</small></div></div></section>
    ...
    <section className="guest-list"><div className="guest-list-heading"><h2>Guest list</h2><div><button disabled={setupRequired || busy} onClick={() => { setBusy(true); refresh().then(() => setError('')).catch(e => setError(e.message)).finally(() => setBusy(false)); }}>Refresh</button><button disabled={setupRequired} onClick={() => fileInput.current?.click()}>Import CSV</button>{!setupRequired ? <a href="/api/admin/guests?format=csv">Export CSV ↓</a> : <button disabled>Export CSV ↓</button>}</div></div><input type="file" ref={fileInput} accept=".csv,text/csv" className="sr-only" tabIndex={-1} onChange={e => readFile(e.target.files?.[0])} aria-label="Import guest CSV" />
      ...
      <div className="guest-table-wrap"><table><thead><tr><th>Guest / party</th><th>Allocated</th><th>RSVP</th><th>Attending</th><th>Dietary needs</th><th>Table</th><th>Invitation</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{visible.map(guest => <tr key={guest.id}><td><strong>{guest.display_name}</strong>{guest.plus_one_name && <small>With {guest.plus_one_name}</small>}<small>{guest.preferred_language === 'th' ? 'Thai' : 'English'}</small></td><td>{guest.seats_allocated}</td><td><span className={`rsvp-badge ${guest.rsvp_status}`}>{guest.rsvp_status}</span></td><td className="attendee-cell">{guest.rsvp_status === 'accepted' ? guest.seats_confirmed : '—'}</td><td className="dietary-cell">{guest.dietary_requirement || '—'}</td><td>{guest.table_number || '—'}</td><td><button onClick={() => copyInvitation(guest)}>Copy link ↗</button></td><td className="row-actions"><button aria-label={`Edit ${guest.display_name}`} onClick={() => edit(guest)}>Edit</button><button aria-label={`Delete ${guest.display_name}`} onClick={() => { setError(''); setDeleting(guest); }}>Delete</button></td></tr>)}</tbody></table></div>
      {!visible.length && <div className="admin-empty"><span aria-hidden="true">♡</span><h3>{setupRequired ? 'A place for every reply.' : guests.length ? 'No matching invitations.' : 'Start with your guest list.'}</h3><p>{setupRequired ? 'Connect the database to see confirmed guests here.' : guests.length ? 'Try another name or RSVP filter.' : 'Add one invitation per person or family, and allocate the seats they can confirm.'}</p>{!setupRequired && !guests.length && <button className="admin-primary" onClick={() => edit()}>Add your first invitation</button>}</div>}
    ...
    <dialog ref={dialog} className="admin-dialog" aria-labelledby="admin-dialog-title" onCancel={e => { if (busy) e.preventDefault(); else closeDialog(); }} onClick={e => { if (e.target === dialog.current) closeDialog(); }}><div><button className="admin-close" aria-label="Close" disabled={busy} onClick={closeDialog}>×</button>
      {editor && <form onSubmit={e => { e.preventDefault(); change(editor === 'new' ? 'POST' : 'PATCH', { ...draft, ...(editor === 'new' ? {} : { id: editor.id }) }, editor === 'new' ? 'Invitation created. Copy its link to send through LINE.' : 'Invitation updated.'); }}><h2 id="admin-dialog-title">{editor === 'new' ? 'Add an invitation' : 'Edit invitation'}</h2><p>One person or family, one personal link.</p><label>Guest or family name<input required maxLength={150} value={draft.display_name} onChange={e => setDraft({ ...draft, display_name: e.target.value })} /></label><div className="admin-form-row"><label>Allocated seats<input type="number" required min={editor === 'new' ? 1 : Math.max(1, editor.seats_confirmed)} max={20} value={draft.seats_allocated} onChange={e => setDraft({ ...draft, seats_allocated: Number(e.target.value) })} /></label><label>Language<select value={draft.preferred_language} onChange={e => setDraft({ ...draft, preferred_language: e.target.value as 'en' | 'th' })}><option value="en">English</option><option value="th">Thai</option></select></label></div><label>Table number <span>(optional)</span><input value={draft.table_number ?? ''} maxLength={30} onChange={e => setDraft({ ...draft, table_number: e.target.value || null })} /></label><label className="admin-checkbox"><input type="checkbox" checked={draft.plus_one_allowed} onChange={e => setDraft({ ...draft, plus_one_allowed: e.target.checked })} />Allow the guest to name a plus-one within their allocated seats</label>{error && <p role="alert" className="admin-error">{error}</p>}<button className="admin-primary" disabled={busy}>{busy ? 'Saving…' : editor === 'new' ? 'Create invitation' : 'Save changes'}</button></form>}
